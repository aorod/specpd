...
export default function FluxoBarChart({ title, icon, data, formatLabel, forceCollapsed }) {
  ...
  return (
    <ChartCard title={title} icon={icon} actions={sortButton} forceCollapsed={forceCollapsed}>
      ...
      <div style={{ position: 'relative', overflowY: entries.length > VISIBLE_ROWS ? 'auto' : 'visible', aspectRatio: entries.length > VISIBLE_ROWS ? `${SVG_W} / ${SCROLL_H}` : undefined }}>
        <svg width="100%" viewBox={`0 0 ${SVG_W} ${svgH}`}>
          {entries.map(([key, { total, fluxoNormal, fluxoER }], i) => {
            ...
            const displayLabel = label.length > 19 ? label.slice(0, 19) + '…' : label;
            ...
            return (
              <g key={key}>
                ...
                {normalW > 0 && (
                  <rect
                    x={barX} y={y} width={normalW} height={ROW_H} rx={3}
                    fill="var(--color-normal)"
                    opacity={tooltip && tooltip.key !== `${key}-n` ? 0.4 : 1}
                    className="h-bar"
                    style={{ animationDelay: `${i * 40}ms` }}
                    onMouseEnter={() => setTooltip({ key: `${key}-n`, label: 'Fluxo Normal', value: fluxoNormal, name: label })}
                    onMouseLeave={() => setTooltip(null)}
                  />
                )}
                {erW > 0 && (
                  <rect
                    x={barX + normalW} y={y} width={erW} height={ROW_H} rx={3}
                    fill="var(--color-er)"
                    opacity={tooltip && tooltip.key !== `${key}-e` ? 0.4 : 1}
                    className="h-bar"
                    style={{ animationDelay: `${i * 40 + 20}ms` }}
                    onMouseEnter={() => setTooltip({ key: `${key}-e`, label: 'Eng. Reversa', value: fluxoER, name: label })}
                    onMouseLeave={() => setTooltip(null)}
                  />
                )}
        ...
        {tooltip && (
          <div className="chart-tooltip">
            <span className="chart-tooltip-label">{tooltip.name}</span>
